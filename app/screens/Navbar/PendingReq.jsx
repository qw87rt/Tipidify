...
import { getPendingRequests, ScreenHeaderBtn, getLeaveType } from '../../../components'; // Import the functions
...
const PendingRequestsScreen = () => {
  const [requests, setRequests] = useState(null); // initial state is null
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        await AsyncStorage.setItem('pendingcount', currentPendingCountStr);
      ...
      <ScrollView>
        {requests && requests.map((request, index) => (
          <View key={index} style={styles.requestContainer}>
            ...
            <Text style={styles.infoText}>        Inclusive Dates: {request.inclusivedates}</Text>
            
            </Text>
            <Text style={styles.infoText}>        Duration: {request.duration} day/s</Text>
            <Text style={styles.infoText}>
              Status:{' '}
              {request.process_no === 1 ? 'Pending @HRLO' : request.process_no === 2 ? 'Pending @HR' : 'Pending'}
            </Text>
          </View>
        ))}
      </ScrollView>
